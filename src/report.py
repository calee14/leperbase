...
from income import *
from models import *
from util.snapshot import store_report
import time
...
def build_report(ticker):
    ...
    report = CompanyReport(report_date, income_report, earnings_report)
    
    print(earnings_report.__dict__, income_report)
...
def test_store():
    ...
    report = CompanyReport('today', IncomeReport.load_json(income_report), EarningsReport.load_json(earnings_report))
    print(report)
print('start')
start = time.time()
build_report('CRWD')
